...
namespace Save_My_Data
{
    public partial class Register : Form
    {
        ...
        private void UserRegister(String UserName, String EMmail, String Security, String Password, String REepassword)
        {
            ...
            {
                Ping ping = new Ping();
                PingReply pingStatus = ping.Send("8.8.8.8"); // Google DNS sunucusu IP adresi
                if (pingStatus.Status == IPStatus.Success)
                {
                    ...
                    {
                        cmd = new MySqlCommand("Insert Into kullanicilar(kullaniciadi,email,kimlikno,sifre) Values ('"
                           + UserName + "','"
                            + EMmail + "','"
                             + Security + "','"
                           + Password + "')", db);
